# LLM Tags

Упрощенная система тегирования обращений клиентов с использованием LLM.

## 🎯 Основные возможности

- 🏷️ **Автоматическое тегирование** обращений клиентов через LLM
- 🚀 **Батчинг** - обработка 30-100 обращений за раз
- ⚡ **Многопоточность** - 5 параллельных запросов к LLM
- 💾 **Кеширование** результатов для повторного прогона
- 🔄 **Фильтрация** по количеству существующих тегов
- 🤖 **Ollama** - использование локальных LLM моделей (qwen32b по умолчанию)
- 🌐 **OpenAI API** - поддержка OpenAI и совместимых API (vLLM, LM Studio, и др.)
- 📊 **Словарь тегов** с описаниями

## 📦 Установка

### Требования

- Python 3.10+
- Ollama (для локального запуска LLM)

### Установка зависимостей

```bash
pip install -r requirements.txt
```

### Настройка Ollama

```bash
# Установите Ollama (если еще не установлена)
# https://ollama.ai/

# Запустите Ollama
ollama serve

# Скачайте модель
ollama pull qwen2.5:32b
```

## 🚀 Быстрый старт

### Базовое использование

```python
import pandas as pd
from llm_tags import TaggingPipeline

# Загрузка данных
df = pd.read_csv("data.csv")  # или pd.read_parquet("data.parquet")

# Создание pipeline
pipeline = TaggingPipeline()

# Промпт для тегирования
prompt = """
Проанализируй обращения клиентов и определи теги для каждого.
Теги должны отражать основную тему обращения.
Формат ответа: JSON массив [{"tag": "название", "description": "описание"}, ...]
"""

# Тегирование
result_df, tags_dict = pipeline.tag(
    df,
    text_column="text",
    tag_prompt=prompt
)

# result_df содержит исходные колонки + колонка "tags" (строка с тегами через запятую)
# tags_dict содержит {tag_name: description}

# Сохранение результатов
result_df.to_csv("tagged_data.csv", index=False)
```

## 📚 Документация

### Класс `OllamaLLM`

Класс для работы с LLM через Ollama (локальные модели).

```python
from llm_tags import OllamaLLM

llm = OllamaLLM(
    api_url="http://localhost:11434/api",  # URL Ollama API
    model="qwen2.5:32b",                    # Название модели
    temperature=0.7                         # Температура генерации
)
```

### Класс `OpenAICompatibleLLM`

Класс для работы с OpenAI-совместимыми API (OpenAI, vLLM, LM Studio и др.).

```python
from llm_tags import OpenAICompatibleLLM

# Пример с OpenAI API
llm = OpenAICompatibleLLM(
    api_url="https://api.openai.com/v1",
    model="gpt-4",
    api_key="your-api-key-here",
    temperature=0.7,
    max_tokens=4096,
    top_p=1.0,
    frequency_penalty=0.0,
    presence_penalty=0.0
)

# Пример с vLLM (локальный сервер)
llm = OpenAICompatibleLLM(
    api_url="http://localhost:8000/v1",
    model="meta-llama/Llama-2-7b-chat-hf",
    temperature=0.7
)

# Пример с LM Studio
llm = OpenAICompatibleLLM(
    api_url="http://localhost:1234/v1",
    model="local-model",
    temperature=0.7
)
```

**Параметры:**
- `api_url` - URL API (обязательный)
- `model` - название модели (обязательный)
- `api_key` - API ключ (опционально, для OpenAI)
- `temperature` - температура генерации (0.0-2.0, по умолчанию 0.7)
- `max_tokens` - максимум токенов в ответе (по умолчанию 4096)
- `top_p` - nucleus sampling (0.0-1.0, по умолчанию 1.0)
- `frequency_penalty` - штраф за повторения (0.0-2.0, по умолчанию 0.0)
- `presence_penalty` - штраф за присутствие токенов (0.0-2.0, по умолчанию 0.0)

#### Метод `tag_batch`

```python
results = llm.tag_batch(
    requests=["текст1", "текст2", ...],     # Список обращений
    prompt="промпт с инструкциями",         # Промпт для тегирования
    existing_tags={"тег1": "описание1"},    # Существующие теги (опционально)
    max_tags=5                              # Максимум тегов на обращение
)
```

### Класс `TaggingPipeline`

Основной класс для тегирования обращений.

```python
from llm_tags import TaggingPipeline

pipeline = TaggingPipeline(
    llm=None,              # Экземпляр OllamaLLM или OpenAICompatibleLLM (если None, создается OllamaLLM по умолчанию)
    batch_size=50,         # Размер батча (30-100 рекомендуется)
    num_workers=5          # Количество параллельных потоков
)
```

#### Метод `tag`

```python
result_df, tags_dict = pipeline.tag(
    df,                           # DataFrame с обращениями
    text_column="text",           # Название колонки с текстом
    tag_prompt="промпт",          # Промпт для тегирования
    existing_tags=None,           # Существующие теги (опционально)
    skip_if_tags_count=10,        # Пропустить строки с >= N тегов
    max_tags=5                    # Максимум тегов на обращение
)
```

**Параметры:**

- `df` - DataFrame с обращениями
- `text_column` - название колонки с текстом обращений
- `tag_prompt` - промпт с инструкциями по тегированию
- `existing_tags` - словарь существующих тегов `{tag_name: description}` (опционально)
- `skip_if_tags_count` - пропустить строки, где количество тегов >= этого значения
  - `10` (по умолчанию) - почти все переразмечаем
  - `0` - обработать только строки без тегов
- `max_tags` - максимальное количество тегов на одно обращение (по умолчанию 5)

**Возвращает:**

- `result_df` - DataFrame с исходной структурой + колонка `tags` (строка с тегами через запятую)
- `tags_dict` - словарь тегов `{tag_name: description}`

## 💡 Примеры использования

### Пример 1: Базовое тегирование

```python
import pandas as pd
from llm_tags import TaggingPipeline

# Данные
df = pd.DataFrame({
    "text": [
        "Не могу войти в личный кабинет",
        "Забыл пароль",
        "Товар не пришел",
    ]
})

# Pipeline
pipeline = TaggingPipeline()

# Промпт
prompt = """
Определи теги для обращений клиентов телеком-оператора.
Категории: авторизация, пароль, доставка, тарифы, технические_проблемы.
"""

# Тегирование
result_df, tags_dict = pipeline.tag(df, text_column="text", tag_prompt=prompt)

print(result_df[["text", "tags"]])
```

### Пример 2: Повторный прогон с существующими тегами

```python
# Первый прогон
result_df1, tags_dict1 = pipeline.tag(df1, text_column="text", tag_prompt=prompt)

# Второй прогон с использованием тегов из первого
result_df2, tags_dict2 = pipeline.tag(
    df2,
    text_column="text",
    tag_prompt=prompt,
    existing_tags=tags_dict1  # Передаем теги из первого прогона
)
```

### Пример 3: Обработка только строк без тегов

```python
# DataFrame с уже проставленными тегами
df = pd.DataFrame({
    "text": ["обращение 1", "обращение 2", "обращение 3"],
    "tags": ["тег1", "тег1, тег2", ""]  # Последнее без тегов
})

# Обработать только строки без тегов
result_df, tags_dict = pipeline.tag(
    df,
    text_column="text",
    tag_prompt=prompt,
    skip_if_tags_count=0  # Пропустить все, где есть хотя бы 1 тег
)
```

### Пример 4: Загрузка из файла

```python
# Загрузка из CSV
df = pd.read_csv("data.csv")

# Или из Parquet
df = pd.read_parquet("data.parquet")

# Или из CSV.GZ
df = pd.read_csv("data.csv.gz", compression="gzip")

# Тегирование
result_df, tags_dict = pipeline.tag(df, text_column="text", tag_prompt=prompt)

# Сохранение результатов
result_df.to_csv("tagged_data.csv", index=False)
```

### Пример 5: Кастомные настройки (Ollama)

```python
from llm_tags import TaggingPipeline, OllamaLLM

# Кастомный LLM
custom_llm = OllamaLLM(
    api_url="http://localhost:11434/api",
    model="qwen2.5:32b",
    temperature=0.5  # Меньше креативности
)

# Pipeline с кастомными настройками
pipeline = TaggingPipeline(
    llm=custom_llm,
    batch_size=30,      # Меньший батч
    num_workers=3       # Меньше параллельных запросов
)

result_df, tags_dict = pipeline.tag(
    df,
    text_column="text",
    tag_prompt=prompt,
    max_tags=3  # Максимум 3 тега
)
```

### Пример 6: Использование OpenAI API

```python
from llm_tags import TaggingPipeline, OpenAICompatibleLLM

# OpenAI LLM
openai_llm = OpenAICompatibleLLM(
    api_url="https://api.openai.com/v1",
    model="gpt-4",
    api_key="your-api-key-here",
    temperature=0.7,
    max_tokens=4096
)

# Pipeline с OpenAI
pipeline = TaggingPipeline(llm=openai_llm)

result_df, tags_dict = pipeline.tag(
    df,
    text_column="text",
    tag_prompt=prompt
)
```

### Пример 7: Использование vLLM (локальный сервер)

```python
from llm_tags import TaggingPipeline, OpenAICompatibleLLM

# Запустите vLLM сервер:
# python -m vllm.entrypoints.openai.api_server \
#     --model meta-llama/Llama-2-7b-chat-hf \
#     --port 8000

# vLLM LLM
vllm_llm = OpenAICompatibleLLM(
    api_url="http://localhost:8000/v1",
    model="meta-llama/Llama-2-7b-chat-hf",
    temperature=0.7
)

# Pipeline с vLLM
pipeline = TaggingPipeline(llm=vllm_llm)

result_df, tags_dict = pipeline.tag(
    df,
    text_column="text",
    tag_prompt=prompt
)
```

### Пример 8: Использование LM Studio

```python
from llm_tags import TaggingPipeline, OpenAICompatibleLLM

# 1. Запустите LM Studio
# 2. Загрузите модель
# 3. Включите локальный сервер (порт 1234)

# LM Studio LLM
lmstudio_llm = OpenAICompatibleLLM(
    api_url="http://localhost:1234/v1",
    model="local-model",
    temperature=0.7,
    timeout=600,
    disable_thinking=True,  # Отключить размышления для некоторых моделей
    use_response_format=False  # LM Studio может не поддерживать response_format
)

# Pipeline с LM Studio
pipeline = TaggingPipeline(llm=lmstudio_llm)

result_df, tags_dict = pipeline.tag(
    df,
    text_column="text",
    tag_prompt=prompt
)
```

## 🔧 Параметры фильтрации

### `skip_if_tags_count`

Определяет, какие строки обрабатывать:

- `skip_if_tags_count=10` (по умолчанию) - пропустить строки с 10+ тегами (почти все переразмечаем)
- `skip_if_tags_count=0` - обработать только строки без тегов
- `skip_if_tags_count=1` - обработать строки с 0 тегами (пропустить с 1+)
- `skip_if_tags_count=2` - обработать строки с 0-1 тегами (пропустить с 2+)

### `max_tags`

Ограничивает количество тегов на одно обращение:

- `max_tags=5` (по умолчанию) - максимум 5 тегов
- `max_tags=3` - максимум 3 тега
- `max_tags=1` - только один тег на обращение

## 💾 Кеширование

Pipeline автоматически сохраняет результаты в кеш (`./cache/tags_cache.csv` по умолчанию).

При повторном запуске:
- Загружаются результаты из кеша
- Обрабатываются только новые или отфильтрованные строки
- Кеш обновляется новыми результатами

**Формат кеша:**
```csv
request_id,tags
req-000001,"тег1, тег2"
req-000002,"тег3"
```

## 📊 Формат данных

### Входные данные

DataFrame должен содержать:
- Колонку с текстом обращений (по умолчанию `text`)
- Опционально: колонку `request_id` (генерируется автоматически если отсутствует)
- Опционально: колонку `tags` (для повторного прогона)

### Выходные данные

**DataFrame:**
- Все исходные колонки
- Колонка `tags` - строка с тегами через запятую (например: "тег1, тег2, тег3")

**Словарь тегов:**
```python
{
    "авторизация": "Проблемы со входом в систему",
    "пароль": "Восстановление или смена пароля",
    "доставка": "Вопросы по доставке товаров"
}
```

## 🎨 Промпты

### Структура промпта

Промпт должен содержать:
1. Описание задачи
2. Категории тегов (опционально)
3. Инструкции по формату ответа

### Пример промпта

```python
prompt = """
Проанализируй обращения клиентов телеком-оператора и определи теги для каждого.

Возможные категории тегов:
- авторизация: проблемы со входом в систему
- пароль: восстановление или смены пароля
- тарифы: вопросы по тарифным планам
- технические_проблемы: неполадки с услугами
- биллинг: вопросы по оплате и списаниям

Можешь создавать новые теги если существующие не подходят.

Формат ответа: JSON массив с объектами {"tag": "название", "description": "описание"}
Максимум 5 тегов на обращение.
"""
```

## ⚙️ Настройка производительности

### Размер батча

- **30-50** - для небольших датасетов или слабых GPU
- **50-70** - оптимальный баланс
- **70-100** - для мощных GPU и больших датасетов

### Количество потоков

- **3** - для GPU с 8GB VRAM
- **5** - оптимальный баланс (по умолчанию)
- **7-10** - для мощных GPU с 16GB+ VRAM

### Температура

- **0.3-0.5** - более детерминированные результаты
- **0.7** - баланс (по умолчанию)
- **0.9-1.0** - более креативные теги

## 🐛 Troubleshooting

### Ошибка подключения к Ollama

```
ConnectionError: Не удалось подключиться к Ollama
```

**Решение:**
1. Убедитесь что Ollama запущена: `ollama serve`
2. Проверьте что модель установлена: `ollama list`
3. Установите модель если нужно: `ollama pull qwen2.5:32b`

### Медленная обработка

**Решение:**
1. Уменьшите `batch_size` (например, до 30)
2. Уменьшите `num_workers` (например, до 3)
3. Используйте более быструю модель

### Ошибки парсинга JSON

**Решение:**
1. Уменьшите `temperature` (например, до 0.5)
2. Улучшите промпт - добавьте больше примеров формата
3. Используйте более мощную модель

## 📝 Примеры

Полные примеры использования см. в файлах:
- [`example.py`](example.py) - примеры с Ollama
- [`openai_example.py`](openai_example.py) - примеры с OpenAI-совместимыми API

Запуск примеров:

```bash
# Примеры с Ollama
python example.py

# Примеры с OpenAI API
python openai_example.py
```

## 🤝 Вклад

Проект создан на базе [llm_clustering](../llm_clustering).

## 📄 Лицензия

MIT

